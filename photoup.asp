...
main_pk = trim(request("main_pk")) '接photo_up.asp檔傳過來的值
%>
...
if Trim(f1)="" then
  ...
  set objfolder = server.createobject("scripting.filesystemobject") 
	...
	success=obj.FormSaveAs(b,"file1",Server.MapPath(".") & foldername & "\" & fname)     '存檔，儲存位置為硬碟上之絕對路徑。此路徑必須存在
	
	if success > 0 then    ' 若成功，傳回大於0之值	
		myStr = "update l8b_ARY_FIB_Data set fib_photo_1='"& fname &"' where main_pk ='" & main_pk & "'"

...
main_pk = trim(request("main_pk")) '接photo_up_photo2.asp傳過來的值
%>
...
' 底下接收檔案
f2=obj.FormFileName(b,"file2")     '  file2  為上一支程式選檔之輸入項名稱
if Trim(f2)="" then
	...
	' 檔案長度
	fsize=obj.FormFileSize(b,"file2")				'檔案長度
	fname=Mid(f2,InstrRev(f2,"\")+1)				'檔案名稱
  ...
  set objfolder = server.createobject("scripting.filesystemobject") 
	...
	success=obj.FormSaveAs(b,"file2",Server.MapPath(".") & foldername & "\" & fname)     '存檔，儲存位置為硬碟上之絕對路徑。此路徑必須存在
	
	if success > 0 then    ' 若成功，傳回大於0之值	
		myStr = "update l8b_ARY_FIB_Data set fib_photo_2='"& fname &"' where main_pk ='" & main_pk & "'"
...
%>
<%
'response.Write("<script>onClick="history.go(-1)"</script>") '回前頁並重新整理
